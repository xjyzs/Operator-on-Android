...
class CmdExecutor private constructor() {
    ...
    private fun startShell() {
        try {
            ...
            val proc = ProcessBuilder("sh").start()
            process = proc
            ...
            closeResources()
            throw IllegalStateException("无法启动进程", e)
        }
    }
    ...
    suspend fun execute(command: String, timeoutMs: Long = 10000L): Result = mutex.withLock {
        withContext(suDispatcher) {
            ...
            val currentWriter = writer ?: return@withContext Result(
                -1,
                "",
                "",
                error = IllegalStateException("Writer 为空")
            )
            val currentStdout = stdoutReader ?: return@withContext Result(
                -1,
                "",
                "",
                error = IllegalStateException("Stdout 为空")
            )
            val currentStderr = stderrReader ?: return@withContext Result(
                -1,
                "",
                "",
                error = IllegalStateException("Stderr 为空")
            )
                ...
                try {
                    ...
                    while (isActive) {
                        ...
                        if (line.startsWith(stdoutBoundary)) {
                            ...
                            exitCode =
                                line.substringAfter(stdoutBoundary).trim().toIntOrNull() ?: -1
                            break
                        }
            ...
            try {
                ...
                Result(
                    -1,
                    "",
                    "命令超时并已重置后台 Shell 进程 ($timeoutMs ms)",
                    isTimeout = true,
                    error = e
                )
            } catch (e: Exception) {
    ...
    companion object {
        @Volatile
        private var instance: CmdExecutor? = null

        fun getInstance(): CmdExecutor {
            return instance ?: synchronized(this) {
                instance ?: CmdExecutor().also { instance = it }
            }
        }
    }
}